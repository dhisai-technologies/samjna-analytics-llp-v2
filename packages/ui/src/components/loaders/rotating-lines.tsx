...
const RotatingLines = React.forwardRef<HTMLSpanElement, RotatingLinesProps>(
    ...
    return (
      <Comp className={cn(spinnerVariants({ size, className: filteredClassName }))} ref={ref} {...props}>
        {Array.from({ length: 8 }).map((_, i) => (
          ...
          >
            <span className={cn("block w-full h-[30%] rounded-full bg-foreground", bgColorClass)} />
          </span>
        ))}
      </Comp>
    );
...
RotatingLines.displayName = "RotatingLines";
