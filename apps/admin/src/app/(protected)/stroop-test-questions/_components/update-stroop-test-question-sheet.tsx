"use client";
import { type StroopTestQuestion, stroopTestLevels } from "@config/stress";
...
import { updateStroopTestQuestion } from "../_lib/actions";
import type { UpdateStroopTestQuestionSchema } from "../_lib/validations";

interface UpdateStroopTestQuestionSheetProps extends React.ComponentPropsWithRef<typeof Dialog> {
  question: StroopTestQuestion;
}

export function UpdateStroopTestQuestionSheet({ question, ...props }: UpdateStroopTestQuestionSheetProps) {
  const form = useForm<UpdateStroopTestQuestionSchema>({
    defaultValues: {
      stroopTestQuestionId: question.id,
      ...
      sourceColor: question.source.color,
  ...
  const [state, action] = useDataFormState(updateStroopTestQuestion, {
    onSuccess: () => {
      form.reset();
  ...
  return (
    <DataFormContainer
      type="sheet"
      title="Update stroop test question"
      description="Modify the details below to update the stroop test question."
